...
def main(args):
	...
	isolates = list(alle_prof_df.index) # to remove
	isolate = args['iso_id'] # to introduce
	...
	if args['miss']:
		...
		out_alle_df = pd.DataFrame(index=[isolate], columns=isolates) #empy df for output # changed
		...
		for isolate_2 in isolates:
			dist = len(alle_prof_df.loc[isolate].compare(alle_prof_df.loc[isolate_2]))
			out_alle_df.at[isolate, isolate_2] = dist
		out_alle_df.to_csv(args['out_dir'] + f'{isolate}.tsv', sep='\t', index=True)
		...
		out_alle_df = pd.DataFrame(index=[isolate], columns=isolates)

		for isolate_2 in isolates:
			dist = len(alle_prof_df.loc[isolate].compare(alle_prof_df.loc[isolate_2])) #keeps missing data
			out_alle_df.at[isolate, isolate_2] = dist
		out_alle_df.to_csv(args['out_dir'] + f'{isolate}.tsv', sep='\t', index=True)

if __name__=='__main__':
	##Create arguments
	parser = argparse.ArgumentParser(description='Script written to create distance matrix from results_alleles.tsv, of some other tab-delimeted allele profile. (for use in snakemake workflow, takes isolate ID  and runs on singular isolate)')
	...
	parser.add_argument(
	'-id', '--isolate_id', dest='iso_id',
	help='Isolate ID for current run'
	)
	parser.add_argument(
