...
def main(args):
	...
	# Load convert tab into lookup dictionary for quick lookup
	md5_lookup = {}
	loci_lookup = {}

	with open(args['con_tab']) as f:
		for line in f:
			md5, loci, allele = line.rstrip().split(',')
			md5_lookup[md5] = (loci, allele)
			allele_num = int(allele.rstrip('_'))
			if loci not in loci_lookup:
				loci_lookup[loci] = []
			loci_lookup[loci].append(allele_num)

	## Functions in script
	# Function to check if md5sum from etoki output is in the database using lookup (etoki misses some of the duplicates)
	def md5_finder(md5):
		if md5 in md5_lookup:
			loci, allele = md5_lookup[md5]
			return True, loci, allele
		return False, None, None
	
	# Function to get to ascertain the number for a novel allele
	def novel_alle_num(loci):
		return max(loci_lookup[loci])

	# Function to add novel allele md5 to convert table database
	def convert_table_update(fheader,novel_alle_,loci_d,loci_lu):
		loci_lu = loci_lu[re.sub('value_md5=','',fheader.split(' ')[1])] = (loci_d, str(novel_alle_) + '_')
			...
			if '>' in line:
				...
				if '1d840e66-127e-c54a-1924-4f29a07fb8b8' in alle_num: # This is the md5sum for 'DUPLICATED'. Take duplicated data is treated as missing
					...
					if md5 and 'dup' not in loci: # If allele (md5) is in reference convert_tab and this loci has not been search yet (not a duplicate)
						...
						md5_lookup[md5] = (loci_dup, str(novel_alle) + '_') # If md5sum for sequence is not found, allele is novel and the md5sum is addeded to the md5sum database.
						...
						if loci_dup in novel_alle_out:
							...
							novel_alle_out[loci_dup] = {novel_alle + '_' + alle_num: isolate_etoki_out_read[counter + 1]}
							md5_lookup[md5] = (loci_dup, str(novel_alle) + '_')
