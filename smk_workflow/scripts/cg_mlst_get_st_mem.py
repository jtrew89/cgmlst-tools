...
def main(args):
	...
	query_df = query_df.map(str)
	query_df = query_df.sort_index(axis=1)
	...
	for isolate in tqdm(isolate_id):
		...
		for profiles_chunk in pd.read_table(args['db'], chunksize=chunk_size, low_memory=False):
			...
			profiles_chunk = profiles_chunk.map(str) #make df str otherwise there is issue with replace
	...
	if not novel_profiles_df.empty:
		...
		shutil.copy2(args['db'], f"{args['db_dir']}")
	else:
		pass
...
if __name__ == '__main__':
	...
	parser.add_argument('-dd', '--db_dir', dest='db_dir', help='Path to and name of ouput file to save under', required=True)
